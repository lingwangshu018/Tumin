...
const source = fs.readFileSync(new URL('../src/App.tsx', import.meta.url), 'utf8') + fs.readFileSync(new URL('../src/MigratedPages.tsx', import.meta.url), 'utf8')
const required = ['onClick={() => go(\'moments\')}', 'onClick={() => go(\'diary\')}', 'onClick={() => go(\'anniversaries\')}', 'chat/completions', 'localStorage.setItem', '确认绑定', '保存设置', '助手设置', '聊天记录', '搜索消息', '收藏消息', '快捷消息', '聊天统计', '导出全部数据']
