...
import { AppearancePage, MemoryLibrariesPage, PromptsPage, ProvidersPage, TranslatorPage } from './SecondBatchPages'

type Page = 'chat' | 'assistant' | 'providers' | 'prompts' | 'memoryLibraries' | 'appearance' | 'translator' | 'history' | 'favorites' | 'search' | 'quick' | 'stats' | 'backup' | 'couple' | 'moments' | 'diary' | 'anniversaries' | 'settings' | 'about'
...
export type Provider = { id: string; name: string; endpoint: string; apiKey: string; models: string[]; enabled: boolean }
export type PromptItem = { id: string; name: string; content: string; enabled: boolean }
export type MemoryLibrary = { id: string; name: string; content: string; enabled: boolean }
export type Appearance = { theme: 'warm' | 'rose' | 'night'; primary: string; userBubble: string; background: string; fontSize: number }
  ...
  return <>
    ...
    <aside className={`sidebar ${open ? 'open' : ''}`}>
      ...
      <nav className="side-nav primary-nav">
        ...
        <button className={page === 'providers' ? 'active' : ''} onClick={() => nav('providers')}><span>◈</span> 服务商与模型</button>
        <button className={page === 'prompts' ? 'active' : ''} onClick={() => nav('prompts')}><span>✦</span> 提示词库</button>
        <button className={page === 'memoryLibraries' ? 'active' : ''} onClick={() => nav('memoryLibraries')}><span>▤</span> 进阶记忆</button>
        <button className={page === 'translator' ? 'active' : ''} onClick={() => nav('translator')}><span>文</span> 翻译</button>
        ...
        <button className={page === 'appearance' ? 'active' : ''} onClick={() => nav('appearance')}><span>◐</span> 显示与主题</button>
...
function ChatPage({ settings, messages, setMessages, quickMessages, prompts, memoryLibraries, onMenu, goSettings }: { settings: Settings; messages: Message[]; setMessages: (m: Message[]) => void; quickMessages: QuickMessage[]; prompts: PromptItem[]; memoryLibraries: MemoryLibrary[]; onMenu: () => void; goSettings: () => void }) {
...
export default function App() {
  ...
  const [providers, setProviders] = useState<Provider[]>(() => stored('tumin.providers', [{ id: 'default', name: 'OpenAI 兼容', endpoint: settings.endpoint, apiKey: settings.apiKey, models: [settings.model], enabled: true }]))
  const [activeProviderId, setActiveProviderId] = useState(() => stored('tumin.activeProviderId', 'default'))
  const [prompts, setPrompts] = useState<PromptItem[]>(() => stored('tumin.prompts', [])); const [memoryLibraries, setMemoryLibraries] = useState<MemoryLibrary[]>(() => stored('tumin.memoryLibraries', []))
  const [appearance, setAppearance] = useState<Appearance>(() => stored('tumin.appearance', { theme: 'warm', primary: '#754c38', userBubble: '#754c38', background: '#fffaf8', fontSize: 14 }))
  ...
  useEffect(() => localStorage.setItem('tumin.settings', JSON.stringify(settings)), [settings]); useEffect(() => localStorage.setItem('tumin.messages', JSON.stringify(messages)), [messages]); useEffect(() => localStorage.setItem('tumin.conversations', JSON.stringify(conversations)), [conversations]); useEffect(() => localStorage.setItem('tumin.quickMessages', JSON.stringify(quickMessages)), [quickMessages]); useEffect(() => localStorage.setItem('tumin.providers', JSON.stringify(providers)), [providers]); useEffect(() => localStorage.setItem('tumin.activeProviderId', JSON.stringify(activeProviderId)), [activeProviderId]); useEffect(() => localStorage.setItem('tumin.prompts', JSON.stringify(prompts)), [prompts]); useEffect(() => localStorage.setItem('tumin.memoryLibraries', JSON.stringify(memoryLibraries)), [memoryLibraries]); useEffect(() => localStorage.setItem('tumin.appearance', JSON.stringify(appearance)), [appearance]); useEffect(() => localStorage.setItem('tumin.moments', JSON.stringify(moments)), [moments]); useEffect(() => localStorage.setItem('tumin.diaries', JSON.stringify(diaries)), [diaries]); useEffect(() => localStorage.setItem('tumin.anniversaries', JSON.stringify(anniversaries)), [anniversaries])
  ...
  const backup = { version: 2, settings, messages, conversations, quickMessages, providers, activeProviderId, prompts, memoryLibraries, appearance, moments, diaries, anniversaries }
  const restore = (data: any) => { if (data.settings) setSettings({ ...defaults, ...data.settings }); if (Array.isArray(data.messages)) setMessages(data.messages); if (Array.isArray(data.conversations)) setConversations(data.conversations); if (Array.isArray(data.quickMessages)) setQuickMessages(data.quickMessages); if (Array.isArray(data.providers)) setProviders(data.providers); if (data.activeProviderId) setActiveProviderId(data.activeProviderId); if (Array.isArray(data.prompts)) setPrompts(data.prompts); if (Array.isArray(data.memoryLibraries)) setMemoryLibraries(data.memoryLibraries); if (data.appearance) setAppearance(data.appearance); if (Array.isArray(data.moments)) setMoments(data.moments); if (Array.isArray(data.diaries)) setDiaries(data.diaries); if (Array.isArray(data.anniversaries)) setAnniversaries(data.anniversaries) }
  const toggleFavorite = (id: string) => setMessages(messages.map(m => m.id === id ? { ...m, favorite: !m.favorite } : m))
  const body = useMemo(() => page === 'chat' ? <ChatPage settings={settings} messages={messages} setMessages={setMessages} quickMessages={quickMessages} prompts={prompts} memoryLibraries={memoryLibraries} onMenu={menu} goSettings={() => setPage('settings')} /> : page === 'assistant' ? <AssistantPage value={settings} save={setSettings} onMenu={menu} /> : page === 'providers' ? <ProvidersPage items={providers} setItems={setProviders} activeId={activeProviderId} activate={(p,m) => { setActiveProviderId(p.id); setSettings({ ...settings, endpoint: p.endpoint, apiKey: p.apiKey, model: m }) }} onMenu={menu} /> : page === 'prompts' ? <PromptsPage items={prompts} setItems={setPrompts} onMenu={menu} /> : page === 'memoryLibraries' ? <MemoryLibrariesPage items={memoryLibraries} setItems={setMemoryLibraries} onMenu={menu} /> : page === 'appearance' ? <AppearancePage value={appearance} save={setAppearance} onMenu={menu} /> : page === 'translator' ? <TranslatorPage settings={settings} onMenu={menu} /> : page === 'history' ? <HistoryPage conversations={conversations} open={openConversation} remove={id => setConversations(conversations.filter(c => c.id !== id))} onMenu={menu} /> : page === 'favorites' ? <FavoritesPage messages={[...messages, ...conversations.flatMap(c => c.messages)]} toggle={toggleFavorite} onMenu={menu} /> : page === 'search' ? <SearchPage messages={messages} conversations={conversations} onMenu={menu} /> : page === 'quick' ? <QuickMessagesPage items={quickMessages} setItems={setQuickMessages} onMenu={menu} /> : page === 'stats' ? <StatsPage current={messages} conversations={conversations} onMenu={menu} /> : page === 'backup' ? <BackupPage data={backup} restore={restore} onMenu={menu} /> : page === 'couple' ? <CoupleHome settings={settings} saveSettings={setSettings} go={setPage} onMenu={menu} /> : page === 'moments' ? <CollectionPage kind="moments" items={moments} setItems={setMoments} onBack={back} onMenu={menu} /> : page === 'diary' ? <CollectionPage kind="diary" items={diaries} setItems={setDiaries} onBack={back} onMenu={menu} /> : page === 'anniversaries' ? <CollectionPage kind="anniversaries" items={anniversaries} setItems={setAnniversaries} onBack={back} onMenu={menu} /> : page === 'settings' ? <SettingsPage value={settings} save={setSettings} onMenu={menu} /> : <AboutPage onMenu={menu} />, [page, settings, messages, conversations, quickMessages, providers, activeProviderId, prompts, memoryLibraries, appearance, moments, diaries, anniversaries])
